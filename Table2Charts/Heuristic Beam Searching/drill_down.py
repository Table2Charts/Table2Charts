...
from Table2Charts.data import DataConfig, State, DataTable, TableQValues, QValue, SpecialTokens, Result, \
...
class BeamDrillDownAgent(Agent):
    def __init__(self, tUID: str, data_config: DataConfig, special_tokens: SpecialTokens, search_config: SearchConfig,
    ...
    def initialize(self):
        if self.load_ground_truth:
            ...
            if len(self.state_actions) == 0:
                self.finished = True
                raise ValueError("No user created valid analysis in {}!".format(self.tUID))
            ...
            self.table = DataTable(self.tUID, self.st, self.data_config)
